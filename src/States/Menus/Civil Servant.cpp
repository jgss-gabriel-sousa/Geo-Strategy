...
CivilServantMenu::CivilServantMenu(GameDataRef data,Country* c,GUIManager* gameGui) : data(data){
    playerCountry = c;
    baseGui = gameGui;
}

void CivilServantMenu::Init(){
    text.setString(data->assets.GetLocalization(data->engine.language,"civilServant"));
    ...
    //Static Menu
    gui.Set("list2","[0]:text="+database->ValueToScale(playerCountry->data.gdp*1000,"best",3));

    gui.Set("list2","[1]:text="+database->ValueToPercentage(playerCountry->data.gdpGrowth,2));
    gui.Set("list2","[1]:style=number");

    gui.Set("list2","[2]:text="+database->ValueToPercentage(playerCountry->lastYearData.gdpGrowth,2));
    gui.Set("list2","[2]:style=number");

    gui.Set("list2","[3]:text="+database->ValueToPercentage(playerCountry->data.inflation,2));
    gui.Set("list2","[3]:style=numberInverse");

    gui.Set("list2","[4]:text="+database->SetPrecision(playerCountry->data.stockExchange,2));
    gui.Set("list2","[4]:style=number");

    gui.Set("list2","[5]:text="+database->SetPrecision(playerCountry->data.exports,2));

    gui.Set("list2","[6]:text="+database->SetPrecision(playerCountry->data.imports,2));

    gui.Set("list2","[7]:text="+database->ValueToPercentage(playerCountry->data.unemployment,2));
    gui.Set("list2","[7]:style=numberInverse");

    gui.Set("list2","[8]:text="+database->SetPrecision(playerCountry->data.unemploymentGrowth,2));
    gui.Set("list2","[8]:style=numberInverse");

    gui.Update();
}

void CivilServantMenu::HandleInput(){
    ;
}

void CivilServantMenu::Update(float dt){
    //if(state)
    SubMenusUpdate(baseGui,&data->machine,&data->input,&data->window,&data->engine.GUIview,&window,&closeButton);
    //gui.Update();
}

void CivilServantMenu::Draw(float dt){
